...
define void @non_mem_use(ptr %addr) {
 ...
 %r = alloca i64
 %r2 = alloca i64
 ; The pointer argument %addr is spilled to shadow stack for GC safety
 ; before each call, since it is a live pointer across the call.
 ; CHECK: i[[PTR]].store {{.+}}, $pop{{.+}}
 ; %r is at SP+24
